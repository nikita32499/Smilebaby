...
export const useFilterItem = (items: IItem[], section: IEntriesSection | undefined) => {
    ...
    const filteredItem = items.filter((item) => {
        ...
        const seasonFilterErr =
            filter.season.length > 0 && !filter.season.includes(item.season.value);

        const countryFilterErr =
            filter.country.length > 0 && !filter.country.includes(item.country.value);

        const sizeFilterErr =
            filter.size.length > 0 &&
            !item.amount.some((amount) => filter.size.includes(amount.size));
